...
MODEL_FILENAME = 'emb_lin_adversarial_resnet_cos_autoenc_cos_m50000_de2en.pkl'
...
#discriminator_0 = Discriminator(d, DISCR_NUM_HIDDEN_LAYERS, DISCR_HIDDEN_DIM, in_dropout_p=0.0, hidden_dropout_p=0.0, hidden2out_dropout_p=0.0, update_hyperparams={'learning_rate': 0.01})
#discriminator_1 = Discriminator(d, DISCR_NUM_HIDDEN_LAYERS, DISCR_HIDDEN_DIM, in_dropout_p=0.0, hidden_dropout_p=0.0, hidden2out_dropout_p=0.0, update_hyperparams={'learning_rate': 0.01})

discriminator_0 = Discriminator(d, DISCR_NUM_HIDDEN_LAYERS, DISCR_HIDDEN_DIM, in_dropout_p=0.0, hidden_dropout_p=0.3, hidden2out_dropout_p=0.1, update_hyperparams={'learning_rate': 0.01})
discriminator_1 = Discriminator(d, DISCR_NUM_HIDDEN_LAYERS, DISCR_HIDDEN_DIM, in_dropout_p=0.0, hidden_dropout_p=0.3, hidden2out_dropout_p=0.1, update_hyperparams={'learning_rate': 0.01})
...
def save_embeddings_to_Klementiev_format(filename, we, vectors=None):
	if vectors == None:
		vectors = we.vectors
	outFs = open(filename, 'w')
	for i, word in enumerate(we.words):
		print >> outFs, word, ':', ' '.join([str(e) for e in vectors[i]])
	outFs.close()

print >> sys.stderr, 'Loading English embeddings...'
we_it = WordEmbeddings()
we_it.load_from_word2vec('./en-m50000')
...
we_en.load_from_word2vec('./de-m50000')
