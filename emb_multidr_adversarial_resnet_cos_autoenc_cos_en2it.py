...
MODEL_FILENAME = 'emb_multidr_adversarial_resnet_cos_autoenc_cos_1_en2it.pkl'
...
class Generator(object):
	def __init__(self, embedding_dim=100, num_hidden_layers=2, hidden_dim=200, latent_dim=10, in_dropout_p=0.2, hidden_dropout_p=0.5, min_std=1e-4, prior_std=0.1, rng=None, update_hyperparams={'learning_rate': 0.01}):
		...
		for i in xrange(self.num_hidden_layers):
			l_enc_hidden = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(self.encoder_layers[-1], num_units=self.num_hidden_layers, nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(gain='relu'), name=('gen_l_enc_hidden_%s' % i)))
		...
		self.gen_l_enc_mu = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(self.encoder_layers[-1], num_units=self.latent_dim, nonlinearity=None, W=lasagne.init.GlorotUniform(), name=('gen_l_enc_mu')))
		...
		for i in xrange(self.num_hidden_layers):
			l_dec_hidden = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(self.decoder_layers[-1], num_units=self.num_hidden_layers, nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(gain='relu'), name=('gen_l_dec_hidden_%s' % i)))
			self.decoder_layers.append(l_dec_hidden)
		self.gen_l_dec_out = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(self.decoder_layers[-1], num_units=self.embedding_dim, nonlinearity=None, W=lasagne.init.GlorotUniform(), name=('gen_l_dec_out')))
		...
		self.get_params_vals = lambda : lasagne.layers.get_all_param_values(self.gen_l_enc_mu, self.gen_l_dec_out)
...
gen_0_updates = lasagne.updates.adam(gen_0_loss, gen_0.gen_params, learning_rate=0.001)
gen_1_updates = lasagne.updates.adam(gen_1_loss, gen_1.gen_params, learning_rate=0.001)

print >> sys.stderr, 'Compiling generators...'
gen_0_eval_fn = theano.function([gen_0.input_var], [gen_0_loss, gen_0.reconstruction_loss, adv_gen_0_loss, adv_0_grad_norm, gen_0.gen_latent, gen_0.gen_prior])
...
gen_0_train_fn = theano.function([gen_0.input_var], [gen_0_loss, gen_0.reconstruction_loss, adv_gen_0_loss, adv_0_grad_norm, gen_0.gen_latent, gen_0.gen_prior], updates=gen_0_updates)
...
accumulators_discr_0 = np.zeros(5)
accumulators_discr_1 = np.zeros(5)
...
def train_batch(batch_id = 1, print_every_n = 1):
	...
	# Generator
	gen_0_loss_val, gen_0_recon_loss_val, gen_0_adv_loss_val, gen_0_grad_norm_val, gen_0_latent_sample, gen_0_prior_sample = gen_0_train_fn(gen_0_X) if not skip_generator_0 else gen_0_eval_fn(gen_0_X)
	...
	accumulators_discr_0_upd = np.array([discr_0_tr_accuracy_val, discr_0_tr_loss_val, discr_0_ev_accuracy_val, discr_0_ev_loss_val, float(skip_discriminator_0)])
	accumulators_discr_1_upd = np.array([discr_1_tr_accuracy_val, discr_1_tr_loss_val, discr_1_ev_accuracy_val, discr_1_ev_loss_val, float(skip_discriminator_1)])
	...
	if batch_id % print_every_n == 0:
		...
		print >> sys.stderr, 'tr acc %s, tr loss: %s, ev acc %s, ev loss: %s, dskip %s, gen loss: %s, recon loss %s, adv loss: %s, grad: %s, gskip: %s' % print_vals_0
		print >> sys.stderr, 'gen 1 ',
		print_vals_1 = tuple(accumulators_discr_1.tolist() + accumulators_gen_1.tolist())
		print >> sys.stderr, 'tr acc %s, tr loss: %s, ev acc %s, ev loss: %s, dskip %s, gen loss: %s, recon loss %s, adv loss: %s, grad: %s, gskip: %s' % print_vals_1
		print >> sys.stderr, ''

def save_model():
	params_vals = lasagne.layers.get_all_param_values([discriminator_0_tr.l_out, discriminator_0_ev.l_out, discriminator_0_tr.l_out, discriminator_0_ev.l_out, ])
