...
MODEL_FILENAME = 'emb_adversarial_6_2_en2it.pkl'
...
gen_train_pred_grad_norm_fn = theano.function([gen_input_var], [gen_loss, deterministic_generation, grad_norm], updates=gen_updates)
gen_eval_pred_grad_norm_fn  = theano.function([gen_input_var], [gen_loss, deterministic_generation, grad_norm])

accumulators = np.zeros(8)
...
def train_batch(batch_id = 1, print_every_n = 1):
	...
	gen_loss_val, X_gen, preout_grad_norm_val = gen_train_pred_grad_norm_fn(X[:HALF_BATCH_SIZE]) if not skip_generator  else gen_eval_pred_grad_norm_fn(X[:HALF_BATCH_SIZE])
	...
	if batch_id == 1:
		accumulators[:] = np.array([accuracy_val, loss_val, alt_accuracy_val, alt_loss_val, gen_loss_val, float(skip_generator), float(skip_discriminator), preout_grad_norm_val])
	else:
		accumulators[:] = ACCUMULATOR_EXPAVG * np.array([accuracy_val, loss_val, alt_accuracy_val, alt_loss_val, gen_loss_val, float(skip_generator), float(skip_discriminator), preout_grad_norm_val]) + (1.0 - ACCUMULATOR_EXPAVG) * accumulators

	if batch_id % print_every_n == 0:
		print >> sys.stderr, 'batch: %s, accuracy: %s, loss: %s, alt acc: %s, alt loss: %s, generator loss: %s, gen skip: %s, disc skip: %s, grad norm: %s' % tuple([batch_id] + accumulators.tolist())
