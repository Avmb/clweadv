...
MODEL_FILENAME = 'emb_adversarial_3_en2it.pkl'
...
l_hid1 = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(l_in, num_units=NUM_HIDDEN1, nonlinearity=lasagne.nonlinearities.leaky_rectify, W=lasagne.init.GlorotUniform(gain=leaky_relu_gain), name='l_hid1'))
l_hid2 = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(l_hid1, num_units=NUM_HIDDEN2, nonlinearity=lasagne.nonlinearities.leaky_rectify, W=lasagne.init.Orthogonal(gain=leaky_relu_gain), name='l_hid2'))
l_preout = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(l_hid2, num_units=1, nonlinearity=None, name='l_preout'))
...
def train_batch(batch_id = 1, print_every_n = 1):
	...
	skip_discriminator = (batch_id > 1) and (accumulators[0] > 0.6)
