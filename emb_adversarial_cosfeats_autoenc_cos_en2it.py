...
MODEL_FILENAME = 'emb_adversarial_cosfeats_autoenc_cos_2_en2it.pkl'
...
X_2 = np.zeros((HALF_BATCH_SIZE, d), dtype=theano.config.floatX)
...
cos_feats = cosine_sim(gen_adversarial_input_var, generation).reshape((-1, 1))
discriminator_input = T.concatenate([generation, cos_feats], axis=1)
discriminator_prediction = lasagne.layers.get_output(discriminator_0.l_out, discriminator_input, deterministic=True)
adv_gen_loss = -T.log(1.0 - discriminator_prediction).mean()
adv_gen_loss.name='adv_gen_loss'

cos_gen_loss = 1.0 - cos_feats.mean()
...
recon_and_cos_gen_updates = lasagne.updates.adam(recon_gen_loss, gen_params, learning_rate=0.001)
...
def train_batch(batch_id = 1, print_every_n = 1):
	...
	gen_loss_val, recon_gen_loss_val, adv_gen_loss_val, cos_gen_loss_val, X_gen, preout_grad_norm_val = gen_train_pred_grad_norm_fn(X[:HALF_BATCH_SIZE], X_2) if not skip_generator  else gen_train_recon_and_cos_only_pred_grad_norm_fn(X[:HALF_BATCH_SIZE], X_2)

	skip_discriminator = (batch_id > 1) and (accumulators[0] > 0.85)
	
	# Discriminator
	X[:HALF_BATCH_SIZE] = X_gen
	loss_val, accuracy_val         = discriminator_0.train_fn(X, X_2, target_mat) if not skip_discriminator else discriminator_0.eval_fn(X, X_2, target_mat)
	alt_loss_val, alt_accuracy_val = discriminator_1.train_fn(X, X_2, target_mat) if not skip_discriminator else discriminator_1.eval_fn(X, X_2, target_mat)
