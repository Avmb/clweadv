...
MODEL_FILENAME = 'emb_adversarial_cos_autoenc_cos_2_en2it.pkl'
...
gen_loss = recon_gen_loss + COS_PENALTY * cos_gen_loss
...
gen_train_pred_fn = theano.function([gen_input_var, gen_adversarial_input_var], [gen_loss, recon_gen_loss, cos_gen_loss, deterministic_generation], updates=gen_updates)
...
gen_eval_pred_fn  = theano.function([gen_input_var, gen_adversarial_input_var], [gen_loss, recon_gen_loss, cos_gen_loss, deterministic_generation])

accumulators = np.zeros(3)
...
def train_batch(batch_id = 1, print_every_n = 1):
	...
	gen_loss_val, recon_gen_loss_val, cos_gen_loss_val, X_gen = gen_train_pred_fn(X[:HALF_BATCH_SIZE], X[HALF_BATCH_SIZE:])
	...
	if batch_id == 1:
		accumulators[:] = np.array([gen_loss_val, recon_gen_loss_val, cos_gen_loss_val])
	else:
		accumulators[:] = ACCUMULATOR_EXPAVG * np.array([gen_loss_val, recon_gen_loss_val, cos_gen_loss_val]) + (1.0 - ACCUMULATOR_EXPAVG) * accumulators

	if batch_id % print_every_n == 0:
		print >> sys.stderr, 'batch: %s, gloss: %s, grloss: %s, gcloss: %s' % tuple([batch_id] + accumulators.tolist())

def save_model():
	params_vals = lasagne.layers.get_all_param_values([gen_l_out])
